...
export class ChatContainerComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.subscription.add(
      this.auth.getUserDate().pipe(filter(data => !!data)).subscribe(user => {
        this.userId = user.uid;
      })
    );
   
    this.subscription.add(
      this.router.events.pipe(filter(ruoterEvent => ruoterEvent instanceof ActivationEnd)).subscribe(data => {
